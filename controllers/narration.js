...
exports.create = [
    ...
    function(req, res) {
        ...
        Account.findById(req.user.id, function(err, account) {
            ...
            console.log("Timeline = %s", template.timeline);
                    ...
                    perspective.save(function(err) {
                        ...
                        Perspective.getPopulated2(perspective, function(err, populatedPerspective) {
                            if (populatedPerspective !== null) {
                                // Replace the generic Frame (e.g., Frame) with Perspective associated with the generic Frame for the current Account
                                moment.frame = populatedPerspective;
                            }

                            io.sockets.emit('narration', moment); // TODO: is this the wrong place?  better place?  guaranteed here?
                            res.json(moment);
                        });
